...
def signup_view(request):
    if request.method == 'GET':
        ...
        return render(request,'success.html',{'signup_form':signup_form})
...
def login_view(request):
    ...
    if request.method == 'GET':
        ...
        if login_form.is_valid():
            ...
            password = login_form.cleaned_dtat['password']
            #read data from db
            user = UserModel.objects.filter(username=username).filter()
